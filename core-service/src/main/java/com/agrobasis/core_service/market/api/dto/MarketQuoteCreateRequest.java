...
public record MarketQuoteCreateRequest(
        @Schema(description = "Commodity cotada", example = "SOYBEAN")
        @NotNull(message = "A commodity é obrigatória")
        Commodity commodity,

        @Schema(description = "Origem da cotação de mercado", example = "CEPEA")
        @NotBlank(message = "A origem da cotação é obrigatória")
        String source,

        @Schema(description = "Preço da commodity", example = "132.45")
        @NotNull(message = "O preço é obrigatório")
        @Positive(message = "O preço deve ser maior que zero")
        BigDecimal price,

        @Schema(description = "Moeda da cotação", example = "USD")
